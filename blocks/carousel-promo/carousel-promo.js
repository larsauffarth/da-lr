...
// moveInstrumentation is optional for Universal Editor support
const moveInstrumentation = (from, to) => {
  // Copy data-aue-* attributes for UE instrumentation if present
  [...from.attributes].filter((attr) => attr.name.startsWith('data-aue-'))
    .forEach((attr) => to.setAttribute(attr.name, attr.value));
};
